...
type Options<T> = {
    ...
    onSelect?: (value: T, anchor: T) => void;
};

export function useDragSelection<T>(options: MaybeGetter<Options<T>>) {
    ...
    const attachment =
        ...
        (element) => {
            ...
            function onpointerdown(e: PointerEvent) {
                const { enabled, isSelectable } = extract(options);
                ...
                timer = setTimeout(() => {
                    ...
                    extract(options).onSelect?.(value, anchorValue);
                }, SELECTION_DELAY);
            }
            ...
            return () => {
                if (anchorElement == element) {
                    if (isSelecting) void Haptics.selectionEnd();
                    suppressClick = false;
                    reset();
                }
